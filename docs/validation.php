<?php
	class Validation
	{
		...
		{ 
			if(preg_match("/^[a-z]([a-z0-9]+|[a-z0-9]+[a-z0-9.]+)*@gmail+\.[a-z]+/",$value))
			{
				$valid=TRUE; 
			}
			else
			...
			if($valid==FALSE)
			{
				$this->errors[]="Email must be  : example@gmail.com";
			}
			return $valid;
		}
		...
		return $valid;
        	...
        	public function exists($value,$fieldname)
        	{
				...
				$upfile = "$DOCUMENT_ROOT/XDG/uploads/".$_FILES[$fieldname]['name'] ;
